...
function cloneSperm(sperm) {
  return {
    ...
    elo: sperm.elo ?? 1000,
    raceHistory: sperm.raceHistory ? [...sperm.raceHistory] : [],
  };
}
...
function submitHabitCheckIn(spermId, rawHabits) {
  ...
  checkIns.set(targetDate, checkInRecord);
  habitCheckInsBySperm.set(spermId, checkIns);

  rebuildTimeline(sperm);

  const snapshot = sperm.history.find((entry) => entry.date === targetDate);
...
function submitRaceResult(spermId, raceData) {
  const sperm = ensureSpermExists(spermId);
  
  const { place, totalRacers = 4, trackId, opponents = [] } = raceData;
  
  if (!Number.isFinite(place) || place < 1 || place > totalRacers) {
    throw createError(400, 'Invalid placement.');
  }
  ...
  const previousElo = sperm.elo ?? 1000;
  sperm.elo = Math.max(0, previousElo + eloChange);
  ...
  if (!sperm.raceHistory) {
    sperm.raceHistory = [];
  }
  sperm.raceHistory.push(raceRecord);
  
  // Keep only last 50 races
  if (sperm.raceHistory.length > 50) {
    sperm.raceHistory = sperm.raceHistory.slice(-50);
  }
...
function getRaceHistory(spermId, limit = 20) {
  const sperm = ensureSpermExists(spermId);
  const cappedLimit = clamp(Number.isFinite(limit) ? Math.round(limit) : 20, 1, 50);
  const history = sperm.raceHistory ?? [];
