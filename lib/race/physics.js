...
function applyControls(state, racerState, dt, context) {
  ...
  if (racerState.finished) {
    racerState.finishingTimer = Math.min(racerState.finishingTimer + dt, 0.6);
    const damping = Math.max(0, 1 - racerState.finishingTimer / 0.6);
    const linvel = body.linvel();
    body.setLinvel({ x: linvel.x * damping, y: linvel.y * damping }, true);
    body.setAngvel(body.angvel() * Math.max(0, 1 - dt * 10), true);
    if (racerState.finishingTimer >= 0.6 && !racerState.parked) {
  ...
  const gapToLeader = context.leaderDistance - racerState.distance;
  if (gapToLeader > context.catchupThreshold) {
    const gain =
  ...
  if (Math.abs(offsetFromCenter) > laneHalfWidth) {
    ...
    if (Math.abs(offsetFromCenter) > laneHalfWidth * 1.15) {
      ...
      body.setLinvel({ x: clippedVel.x * 0.35, y: clippedVel.y * 0.35 }, true);
    }
  }
...
export function stepPhysics(state, dt) {
  ...
  const catchupThreshold = Math.max(160, state.totalLength * 0.1);
