...
function applyControls(state, racerState, dt, context) {
  ...
  if (racerState.finished) {
    ...
    racerState.finishingTimer = Math.min(
      racerState.finishingTimer + dt,
      duration,
    );
    const tRaw = duration > 0 ? racerState.finishingTimer / duration : 1;
    ...
    const travelDistance =
      ease * (glide?.distance ?? state.finishRunout ?? 180);
    const finishOffset =
    ...
    body.setTranslation(
      {
        x:
          ...
          finishTangent.x * travelDistance,
        y:
          origin.y +
          finishNormal.y * finishOffset +
          finishTangent.y * travelDistance,
      },
      true,
    );

    const baseSpeed = glide?.initialSpeed ?? 32;
    const minSpeed = glide?.minSpeed ?? Math.max(2, baseSpeed * 0.15);
    const speedAlong = Math.max(
      minSpeed,
      baseSpeed * (1 - ease * 0.9),
    );
    body.setLinvel(
      {
        x: finishTangent.x * speedAlong,
        y: finishTangent.y * speedAlong,
      },
      true,
    );
    body.setAngvel(0, true);

    if (!racerState.parked && t >= 0.995) {
  ...
  if (!racerState.finished && leaderFinished && remainingDistance < assistReach) {
    const urgency = clamp((assistReach - remainingDistance) / assistReach, 0, 1);
    if (speedForward < targetMaxSpeed * 0.4) {
      forwardAccel += params.thrust * (0.4 + 0.8 * urgency);
    }
  }
...
export function stepPhysics(state, dt) {
  ...
  state.racers.forEach((racerState) => {
    ...
    if (
      ...
      if (remaining > 0) {
        const assistGain = clamp(dt * 3.2, 0, 0.45);
    ...
    if (racerState.finished) {
      ...
      if (racerState.finishHold >= 0.25) {
        ...
        if (settleBody) {
          ...
          const forwardSpeed =
            lv.x * finishTangent.x + lv.y * finishTangent.y;
          const lateralSpeed =
            lv.x * finishNormal.x + lv.y * finishNormal.y;
          initialSpeed = Math.max(10, forwardSpeed);
          if (forwardSpeed < 6 && Math.abs(lateralSpeed) > 4) {
            initialSpeed = Math.max(initialSpeed, Math.abs(lateralSpeed) * 0.6);
          }
          minSpeed = Math.max(2, Math.min(initialSpeed * 0.2, 8));
          settleBody.setLinvel(
            {
              x: finishTangent.x * initialSpeed,
              y: finishTangent.y * initialSpeed,
            },
            true,
          );
