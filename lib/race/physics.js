...
export function stepPhysics(state, dt) {
  ...
  // Detect collisions and overtakes between racers
  const collisionThreshold = 12; // Distance threshold for collision detection
  const overtakeThreshold = 25; // Distance threshold for close proximity
  ...
  for (let i = 0; i < state.racers.length; i++) {
    for (let j = i + 1; j < state.racers.length; j++) {
      ...
      // Overtake detection - check if positions swapped
      const distanceDiff = Math.abs(racer1.distance - racer2.distance);
      if (distance < overtakeThreshold && distanceDiff < 50) {
        const overtakeKey = `${racer1.id}-${racer2.id}`;
        const wasAhead1 = state.overtakePositions?.get(overtakeKey);
        const isAhead1 = racer1.distance > racer2.distance;
        
        // Check if position changed (overtake happened)
        if (wasAhead1 !== undefined && wasAhead1 !== isAhead1) {
          const overtaker = isAhead1 ? racer1 : racer2;
          const overtaken = isAhead1 ? racer2 : racer1;
          
          // Only trigger overtake dialogue 40% of the time to avoid spam
          if (Math.random() < 0.4) {
            overtaker.overtakeEvents.push({ 
              otherRacer: overtaken,
              isOvertaker: true,
            });
            overtaken.overtakeEvents.push({ 
              otherRacer: overtaker,
              isOvertaker: false,
            });
          }
        }
        ...
        if (!state.overtakePositions) state.overtakePositions = new Map();
        state.overtakePositions.set(overtakeKey, isAhead1);
      }
    }
  }
