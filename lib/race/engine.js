...
export async function createRaceEngine(racers, track, cfg = {}, balance) {
  ...
  function step(dt) {
    const allFinished =
      physicsState.placements.length === physicsState.racers.length;
    const animationsDone =
      allFinished &&
      physicsState.racers.every((racer) => racer.finishAnimationDone);
    if (animationsDone) {
      state.finishHoldTimer = Math.min(
        physicsState.postFinishHold ?? 0,
        state.finishHoldTimer + dt,
      );
      if (
        state.finishHoldTimer >= (physicsState.postFinishHold ?? 0) &&
        state.lastFrame
      ) {
        return state.lastFrame;
      }
    } else {
      state.finishHoldTimer = 0;
    }
    ...
    physicsState.racers.forEach((racerState) => {
      ...
      if (racerState.collisionEvents?.length > 0) {
        racerState.collisionEvents.forEach((collision) => {
          events.push({
            t: state.t,
            racerId: racerState.id,
            kind: 'collision',
            otherRacerId: collision.otherRacer.id,
            otherRacerName: collision.otherRacer.name,
            isAggressor: collision.isAggressor,
          });
        });
      }
      // Handle overtake events
      if (racerState.overtakeEvents?.length > 0) {
        racerState.overtakeEvents.forEach((overtake) => {
          events.push({
            t: state.t,
            racerId: racerState.id,
            kind: 'overtake',
            otherRacerId: overtake.otherRacer.id,
            otherRacerName: overtake.otherRacer.name,
            isOvertaker: overtake.isOvertaker,
          });
        });
      }
    });
    ...
    const lanes = physicsState.racers.map((racerState) => {
      ...
      parked: racerState.parked,
      finishingTimer: racerState.finishingTimer,
      };
    });
